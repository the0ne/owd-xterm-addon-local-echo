...
export default class LocalEchoController {
   ...
   */
  read(prompt, continuationPrompt = "> ", callback) {
    return new Promise((resolve, reject) => {
      ...
      this._activePrompt = {
        ...
        resolve: () => {
          if (typeof callback === 'function') {
            callback(this.term, this._input)
          }
      ...
      this._cursor = 0;
      this._active = true;
    }).then(() => this.read(prompt, continuationPrompt, callback));
  }
        ...
        case "\t": // TAB
          if (this._autocompleteHandlers.length > 0) {
            ...
            if (candidates.length === 0) {
              ...
              const sameFragment = getSharedFragment(getLastToken(inputFragment), candidates);
